...
For this implementation, each component is implemented as a module and each channel is a function exported from the module. The controller exports a _send_ method that accepts an action, and the renderer/view exports a _render_ method that accepts a state. Each module also exports a _subscriptions_ array -- each component sequentially call every function within in this array with a it's result (an state or an action, recpectively) when an update takes place.

```
 -(actions)-> [controller] -(state)-> [renderer/view] -(actions)->
```
...
subs.push(action => {
switch(action.type){
  case 'A' : sendA(action); break;
  case 'B' : sendB(action); break;
  default  : sendC(action); break;
  }
  return action;
});
subsA.push(render);
...
Consider what would be needed to add custom storage for the state of our application, as well as logging and authentication for our actions.
...
viewSubscriptions.push(action => log(action).then(authenticate).then(send));
render(initialState);
```
